...
class ClinicAgent(BaseVoiceAgent):
    ...
    async def handle_tool(self, tool_name: str, tool_input: dict[str, Any]) -> Any:
        ...
        business_name = self.business.get("name", "Clinic")

        # ── Auto-inject business_id into tool input so Claude doesn't need to provide it ──
        enriched_input = {**tool_input, "business_id": business_id, "business_name": business_name}

        logger.info(
            "clinic_agent.tool_call",
            tool=tool_name,
            input=tool_input,
        ...
        if tool_name in appointment_tools:
            ...
            logger.warning("clinic_agent.unknown_tool", tool=tool_name)
